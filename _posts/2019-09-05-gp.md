...
Mathematically, $\mathbold{X} = (X_1, ...X_k)^T$ has a multivariate Gaussian distribution if $Y=a_1X_1 + a_2X_2 ... + a_kX_k$ is normally distributed for any constant vector $\mathbold{a} \in \mathcal{R}^k$.



_**Note**: if all k components are Gaussian random variables, then $\mathbold{X}$ must be multivariate Gaussian (because the sum of Gaussian random variables is always Gaussian)._
...
If all components of $\mathbold{X}$ are independent, $\mathbold{X}$ is jointly normal. However, jointly normal components are only independent if they are not correlated.
...
By drawing a verticle red line we fix the value of $y_1$ to compute the density of y2 along the line -- thereby condition on $y_1$. Note that in here since $y_2 \in \mathcal{N}(\mu^*, \sigma_*)$ , by conditioning we get a gaussian back.
...
The covariance matrices in all above examples are computed using the radial basis function (RBF) kernel K(x1, x2) -- all by taking integer values for x1, x2. This RBF kernel ensures the **"smoothness"** of the covariance matrix, by generating a large function value for x1 and x2 that are closer to each other and small value for the ones that are further away. Note that if x1=x2, K=\sigma^2. We then take K and add $I\sigma_y^2$ for the final covariance matrix to factor in noise -- more on this later.

This means in principle, **we can calculate this covariance matrix for any real-valued x1 and x2 by simply plugging them in**. The real-valued xs effectively result in an infinite-dimensional Gaussian defined by the covariance matrix. 
...
Luckily, because $p(y \given \theta)$ is Gaussian, we can compute its likelihood in close form. That means we can just maximise the likelihood of $p(y|\theta)$ under these hyperparameters using a gradient optimiser:
